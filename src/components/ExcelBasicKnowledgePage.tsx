...
import {
  ...
  EXCEL_SHORTCUTS,
  type ExcelTable,
} from "@/content/excelBasicKnowledgeContent";
...
function FormulaBlock({ formula }: { formula: string }) {
  return (
    <div className="max-w-full overflow-x-auto rounded-lg border border-border bg-muted/40 [-webkit-overflow-scrolling:touch]">
      <pre className="max-w-max min-w-0 px-3 py-2.5 font-mono text-xs leading-relaxed whitespace-pre text-foreground sm:px-4 sm:text-sm">
        {formula}
      </pre>
    </div>
  );
...
function DataTable({ table }: { table: ExcelTable }) {
  return (
              ...
              {row.map((cell, j) => (
                <td
                  ...
                  className={cn(
                    ...
                    cell.startsWith("=")
                      ? "font-mono text-xs whitespace-nowrap sm:text-sm"
                      : "whitespace-nowrap",
                  )}
                >
                  {cell}
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-2 overflow-x-auto pb-0.5 [-webkit-overflow-scrolling:touch] sm:flex-wrap sm:overflow-visible">
        {EXCEL_CHAPTER_CARDS.map((card, index) => (
...
function ChapterSection({
  ...
  chapter: (typeof EXCEL_CHAPTERS)[number];
  showMcqs?: boolean;
    ...
    >
      ...
      {chapter.formulas && chapter.formulas.length > 0 && (
        <div className="space-y-3">
          <h3 className="text-lg font-semibold leading-snug text-foreground">Formula Examples</h3>
          <div className="space-y-2">
            {chapter.formulas.map((f) => (
              <FormulaBlock key={f} formula={f} />
            ))}
          </div>
        </div>
      )}
      ...
      {showShortcuts && (
        <div className="space-y-3">
          <h3 className="text-lg font-semibold leading-snug text-foreground">Excel Shortcuts</h3>
          <div className="max-w-full overflow-x-auto rounded-lg border border-border [-webkit-overflow-scrolling:touch]">
      ...
      {showFinalProject && (
        <InfoBox title={EXCEL_FINAL_PROJECT.title} variant="primary">
          <p className="mb-3 text-base leading-relaxed text-muted-foreground md:leading-7">
            {EXCEL_FINAL_PROJECT.description}
          </p>
          <ol className="list-decimal space-y-2.5 pl-5 text-base leading-relaxed text-muted-foreground md:leading-7">
            {EXCEL_FINAL_PROJECT.steps.map((step) => (
              <li key={step}>{step}</li>
            ))}
          </ol>
        </InfoBox>
...
export function ExcelBasicKnowledgePage() {
  return (
    <PageShell>
      <article className="mx-auto min-w-0 max-w-[56rem] space-y-6 overflow-x-hidden font-hindi md:space-y-8">
        ...
        <section aria-labelledby="excel-chapter-cards-heading">
          <h2
            id="excel-chapter-cards-heading"
        ...
        <div className="space-y-14 border-t border-border pt-8 md:space-y-16 md:pt-10">
          {EXCEL_CHAPTERS.map((chapter) => (
            <ChapterSection
              key={chapter.anchor}
              chapter={chapter}
              showMcqs={chapter.anchor === "chapter-6"}
              showShortcuts={chapter.anchor === "chapter-6"}
              showFinalProject={chapter.anchor === "chapter-6"}
            />
          ))}
        </div>
        ...
        <Card className="border-border bg-card/80">
          ...
          <CardContent className="flex flex-col gap-3 sm:flex-row sm:flex-wrap">
            {EXCEL_NEXT_PRACTICE.map((item) => (
              <a
                key={item.label}
                href={`#${item.anchor === "chapter-6" && item.label.includes("MCQ") ? "mcq-practice" : item.anchor}`}
                className={navLinkClass(
                  "outline",
