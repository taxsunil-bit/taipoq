...
import { WORD_BASICS_MODULE } from "@/content/wordBasicKnowledgeContent";
  ...
  return (
    <PageShell>
      <div className="mx-auto max-w-3xl space-y-6 font-hindi">
        ...
        <section aria-labelledby="cb-special-modules-heading">
          <h2 id="cb-special-modules-heading" className="mb-4 text-xl font-bold text-foreground">
            विशेष Module
          </h2>
          <ul className="space-y-3">
            <li>
              <Card className="border-primary/30 bg-primary/5 transition-colors hover:border-primary/50">
                <CardContent className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
                  <div className="min-w-0 flex-1">
                    <p className="text-sm text-muted-foreground">{WORD_BASICS_MODULE.titleHindi}</p>
                    <p className="mt-1 text-base font-semibold leading-snug text-foreground md:text-lg">
                      {WORD_BASICS_MODULE.title}
                    </p>
                    <p className="mt-2 text-sm leading-relaxed text-muted-foreground md:text-base">
                      {WORD_BASICS_MODULE.description}
                    </p>
                  </div>
                  <Link to={WORD_BASICS_MODULE.href} className={navLinkClass("default", "shrink-0")}>
                    {WORD_BASICS_MODULE.buttonLabel}
                  </Link>
                </CardContent>
              </Card>
            </li>
