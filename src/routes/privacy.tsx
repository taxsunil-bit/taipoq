...
import { CalendarDays, Cookie, Link2, Mail, Shield, BarChart3, Eye, Settings2, HardDrive } from "lucide-react";
...
export const Route = createFileRoute("/privacy")({
  head: () => ({
    meta: [
      { title: "Privacy — TAIPOQ" },
      {
        name: "description",
        content:
          "Privacy on TAIPOQ — browser-local profile and progress, cookie preferences, optional analytics, and official-source links.",
      },
    ],
  }),
        ...
        <div className="space-y-6">
          {PRIVACY_POLICY_SECTIONS.filter((s) => s.id !== "intro").map((section) => {
            ...
            return (
              <article
                key={section.id}
                className="rounded-xl border border-border bg-card p-6 shadow-[var(--shadow-subtle)] md:p-8"
              >
